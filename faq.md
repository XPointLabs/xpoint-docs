...
как отдельный обязательный для v1 режим, но пока не имеет verified peer
implementation и безопасно отклоняется при запуске. User-managed/on-prem
профиль выполняется позднее.
