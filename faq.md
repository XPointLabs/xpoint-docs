...
как отдельный режим, но перенесён в следующий спринт после первого
production-релиза. Пока он не имеет verified peer implementation и безопасно
отклоняется при запуске. User-managed/on-prem профиль выполняется позднее.
